...
import { PROJECTS, TEAM_MEMBERS, generateId } from "@/lib/data";
import type { Issue, Priority, Project } from "@/lib/data";
...
export function NewIssueModal({ open, onClose, onSubmit }: NewIssueModalProps) {
  ...
  const handleSubmit = () => {
    ...
    toast({ title: "Issue created", description: `${title} has been added successfully.` });
    reset();
    onClose();
  };
      ...
      <DialogContent className="bg-card border-border max-w-lg">
        ...
        <div className="space-y-4 mt-2">
          <div>
            ...
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Issue title" className={`bg-muted border-border mt-1 ${errors.title ? "border-destructive" : ""}`} />
          ...
          <div>
            ...
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the issue..." className={`bg-muted border-border mt-1 min-h-[80px] ${errors.description ? "border-destructive" : ""}`} />
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="ghost" onClick={() => { reset(); onClose(); }} className="text-muted-foreground">Cancel</Button>
            <Button onClick={handleSubmit} className="bg-primary hover:bg-primary/90 text-primary-foreground">Create Issue</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
